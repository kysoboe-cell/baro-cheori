import type { Company } from "../types";

export const netflix: Company = {
  slug: "netflix",
  name: "넷플릭스",
  categoryId: "subscription",
  aliases: ["넷플릭스", "netflix", "넷플"],

  overview: [
    "넷플릭스는 앱만 지운다고 해지되는 게 아니라서, 계정 화면에서 직접 멤버십 해지를 눌러야 다음 결제가 멈춘다. 그런데도 해지 버튼을 못 찾아 헤매거나, 분명히 해지 절차를 밟았다고 생각했는데 다시 결제된 경우, 카드를 바꾸려고 들어갔더니 결제 화면에 넷플릭스가 아닌 통신사나 앱스토어가 떠 있는 경우가 있다.",
    "이럴 때는 채팅 상담으로 넘어가기 전에 계정 화면부터 확인하는 게 먼저다. 멤버십 상태와 다음 결제 날짜, 결제 상세에서 실제로 빠져나간 날짜와 금액을 본 뒤, 결제 업체가 넷플릭스인지 통신사·애플·구글인지까지 확인해 둔다. 결제 업체가 넷플릭스가 아니면 넷플릭스 쪽에서는 처리할 수 있는 게 없어서, 그 업체 쪽에서 따로 확인해야 한다.",
    "헷갈리기 쉬운 지점은 해지 버튼 자체다. 계정 화면 맨 아래 '멤버십 해지'를 누른 뒤 '해지 +'를 펼치면 검은 해지 완료 버튼이 나오는데, 옆의 파란 버튼이 아니라 이 검은 버튼을 눌러야 실제로 해지된다. 프로필 삭제나 로그아웃, 앱 삭제만 하고 해지됐다고 착각하기도 한다.",
  ],

  services: [
    {
      slug: "membership-cancel",
      title: "멤버십 해지·다음 결제 막기",
      intro: [
        "넷플릭스 앱에서 프로필 삭제나 로그아웃, 앱 삭제만으로는 다음 결제가 멈추지 않는다. 계정 화면을 맨 아래까지 내리면 멤버십 해지 바로 아래 계정 삭제 버튼이 있는데, 눌러야 할 건 멤버십 해지 쪽이다. '멤버십을 직접 관리하세요' 화면에서 해지 +를 펼치면 파란 버튼과 검은 해지 완료 버튼이 같이 나오는데, 진짜 해지를 확정하는 건 검은 버튼이다.",
        "카드가 아니라 통신사나 앱스토어를 통해 결제해온 경우라면 계정 화면에 해지 버튼 자체가 없을 수 있다. 이때는 계정 화면의 결제 정보를 먼저 확인해 결제 업체 쪽에서 해지해야 한다. 해지가 정상 처리되면 화면에 \"○월 ○일부터 효력이 발생합니다\"라는 문구가 뜨고, 그날까지는 이용이 유지된다.",
      ],
      keywords: [
        "넷플릭스 해지",
        "넷플릭스 구독 취소",
        "넷플릭스 자동결제 해지",
        "넷플릭스 다음달 결제 막기",
        "넷플릭스 탈퇴",
      ],
      quickSummary: [
        "넷플릭스 계정의 [멤버십 해지]를 눌러야 다음 결제가 멈춰요. 앱만 지우면 해지되지 않습니다.",
        "해지해도 이미 결제한 이용기간이 끝날 때까지는 볼 수 있어요.",
      ],
      steps: [
        "넷플릭스 앱 오른쪽 아래 [나의 넷플릭스]를 누릅니다.",
        "왼쪽 위 프로필 이름(▾)을 누릅니다.",
        "프로필 창에서 [계정]을 누릅니다.",
        "계정 화면을 맨 아래까지 내립니다.",
        "맨 아래 [멤버십 해지]를 누릅니다. 바로 아래 [계정 삭제]는 누르지 마세요.",
        "'멤버십을 직접 관리하세요' 화면에서 [해지 +]를 누릅니다.",
        "펼쳐진 검은 [해지 완료] 버튼을 누릅니다. 파란 버튼이 아니라 검은 버튼이 진짜 해지입니다.",
        "화면에 \"○월 ○일부터 효력이 발생합니다\"가 보이면 끝. 그날까지는 계속 볼 수 있어요.",
      ],
      keyStepIndexes: [0, 4, 6],
      tips: [
        "[해지] 버튼이 없다면 카드가 아니라 통신사·앱스토어 같은 결제 업체에서 해지해야 할 수 있어요. 계정 화면의 결제 정보를 먼저 보세요.",
        "프로필 삭제, 로그아웃, 앱 삭제는 멤버십 해지가 아니에요.",
      ],
      officialUrl: "https://www.netflix.com/cancelplan",
      officialActionLabel: "넷플릭스 해지 화면 열기",
      officialLinkType: "login",
      screenshotGuide: {
        checkedAt: "2026-08-20",
        basis: "넷플릭스 안드로이드 앱 화면 기준",
        steps: [
          {
            n: 1,
            img: "/images/guides/netflix-cancel/step1.webp",
            thumb: "/images/guides/netflix-cancel/step1-thumb.webp",
            label: "나의 넷플릭스",
            caption: "앱 오른쪽 아래 [나의 넷플릭스]를 누릅니다.",
            alt: "넷플릭스 앱 홈 화면. 하단 탭 오른쪽 끝 '나의 넷플릭스'에 표시",
            linkedStep: 1,
            howToImage: true,
          },
          {
            n: 2,
            img: "/images/guides/netflix-cancel/step2.webp",
            thumb: "/images/guides/netflix-cancel/step2-thumb.webp",
            label: "프로필 이름",
            caption: "왼쪽 위 프로필 이름(▾)을 누릅니다.",
            alt: "나의 넷플릭스 화면. 왼쪽 위 프로필 아바타와 이름, 아래 화살표에 표시",
            linkedStep: 2,
          },
          {
            n: 3,
            img: "/images/guides/netflix-cancel/step3.webp",
            thumb: "/images/guides/netflix-cancel/step3-thumb.webp",
            label: "계정",
            caption: "프로필 창에서 [계정]을 누릅니다.",
            alt: "프로필 창. '앱 설정' 아래 '계정' 메뉴에 표시",
            linkedStep: 3,
          },
          {
            n: 4,
            img: "/images/guides/netflix-cancel/step4.webp",
            thumb: "/images/guides/netflix-cancel/step4-thumb.webp",
            label: "아래로 스크롤",
            caption: "계정 화면을 맨 아래까지 내립니다.",
            alt: "계정 화면. 멤버십 정보와 보안 항목, 아래로 내리라는 화살표",
            linkedStep: 4,
          },
          {
            n: 5,
            img: "/images/guides/netflix-cancel/step5.webp",
            thumb: "/images/guides/netflix-cancel/step5-thumb.webp",
            label: "멤버십 해지",
            caption:
              "맨 아래 [멤버십 해지]를 누릅니다. 바로 아래 [계정 삭제]는 누르지 마세요.",
            alt: "계정 화면 맨 아래. 흰색 '멤버십 해지' 버튼에 표시, 그 아래 '계정 삭제' 버튼",
            linkedStep: 5,
            howToImage: true,
          },
          {
            n: 6,
            img: "/images/guides/netflix-cancel/step6.webp",
            thumb: "/images/guides/netflix-cancel/step6-thumb.webp",
            label: "해지 +",
            caption: "'멤버십을 직접 관리하세요' 화면에서 [해지 +]를 누릅니다.",
            alt: "멤버십 관리 화면. '멤버십 변경' 아래 '해지' 카드의 + 표시에 표시",
            linkedStep: 6,
          },
          {
            n: 7,
            img: "/images/guides/netflix-cancel/step7.webp",
            thumb: "/images/guides/netflix-cancel/step7-thumb.webp",
            label: "해지 완료",
            caption:
              "펼쳐진 검은 [해지 완료] 버튼이 진짜 해지입니다. 누르면 바로 접수돼요.",
            alt: "해지 카드가 펼쳐져 검은색 '해지 완료' 버튼이 보이는 화면",
            linkedStep: 7,
            howToImage: true,
          },
        ],
      },
      faq: [
        {
          question: "해지했는데 왜 앱이 계속 보이나요?",
          answer:
            "결제 주기가 끝나는 날까지는 이용이 유지되는 게 정상이에요. 해지했다고 즉시 접속이 끊기는 게 아니에요.",
        },
        {
          question: "해지를 취소하고 싶어요",
          answer:
            "결제일 전이라면 계정 화면에서 멤버십 재시작이 대부분 가능해요.",
        },
        {
          question: "통신사 결합으로 가입했는데 해지가 안 돼요",
          answer:
            "통신사 제휴 상품은 넷플릭스 앱이 아니라 가입한 통신사 쪽에서 해지해야 하는 경우가 있어요.",
        },
        {
          question:
            "해지 버튼 옆의 계정 삭제를 누르면 어떻게 되나요?",
          answer:
            "계정 삭제는 멤버십 해지와 전혀 다른 기능이에요. 시청 기록과 프로필까지 함께 정리되기 때문에 나중에 다시 볼 생각이라면 누르지 않는 편이 좋습니다. 두 버튼이 화면에서 붙어 있으니 누르기 전에 글자를 한 번 더 읽어보세요.",
        },
        {
          question:
            "가족이 프로필을 나눠 쓰고 있는데 해지하면 다 끊기나요?",
          answer:
            "멤버십은 계정 단위로 적용되기 때문에 같은 계정 안의 프로필은 종료 날짜에 함께 끝납니다. 각자 다른 프로필을 쓰고 있어도 마찬가지예요. 함께 보는 사람이 있다면 언제까지 볼 수 있는지 미리 알려두면 서로 당황할 일이 줄어듭니다.",
        },
        {
          question:
            "해지했는데 다음 결제일이 그대로 표시돼요",
          answer:
            "마지막 검은 [해지 완료] 버튼까지 눌러야 신청이 접수됩니다. 화면에 언제부터 효력이 발생하는지 문구가 떴는지 확인해 보세요. 그 문구를 본 적이 없다면 중간에 멈춘 것이므로 계정 화면에서 다시 진행하면 됩니다.",
        },
      ],

      pitfalls: {
        heading: "여기서 많이 헷갈립니다",
        items: [
          {
            title: "확정 버튼은 파란색이 아니라 검은색입니다",
            body:
              "'멤버십을 직접 관리하세요' 화면에서 [해지 +]를 펼치면 버튼이 두 개 나옵니다. 색이 진하고 큰 파란 버튼이 먼저 눈에 들어오지만, 해지를 확정하는 것은 그 아래 검은 [해지 완료] 쪽이에요.",
          },
          {
            title: "해지 버튼 자체가 없는 계정도 있습니다",
            body:
              "통신사나 앱스토어를 거쳐 결제하고 있으면 넷플릭스 계정 화면에 해지 항목이 나타나지 않을 수 있어요. 이때는 계정 화면의 결제 정보를 먼저 확인해 실제로 돈이 나가는 곳에서 해지해야 합니다.",
          },
          {
            title: "앱을 지우는 것으로는 결제가 멈추지 않습니다",
            body:
              "프로필 삭제, 로그아웃, 앱 삭제는 결제와 관계가 없습니다. 결제는 계정에 걸려 있어서 화면에서 앱이 사라져도 그대로 청구돼요. 반드시 계정 화면에서 해지를 끝내야 합니다.",
          },
        ],
      },

      guideLink: {
        slug: "subscription-cancel-checklist",
        text:
          "다음 결제일과 환불 기준처럼 누르기 전에 볼 것들은 따로 정리해 두었습니다 —",
      },

      lastChecked: "2026-08-20",
    },
    {
      slug: "charged-after-cancel",
      title: "해지했는데 결제됨",
      intro: [
        "넷플릭스 해지 후에도 결제 내역이 남는 경우는 원인이 두 갈래로 나뉩니다. 하나는 해지 신청일과 실제 해지 완료일이 어긋나 다음 결제일 전까지 해지가 미처 끝나지 않은 경우이고, 다른 하나는 본인이 아닌 다른 사람이 같은 계정으로 멤버십을 다시 시작한 경우예요. 원인에 따라 처리 방법이 달라지기 때문에 계정의 멤버십 상태와 결제 상세에서 실제로 빠져나간 날짜·금액부터 확인해 두 경우를 갈라봐야 합니다.",
        "카드 명세에 '넷플릭스'가 아니라 통신사·애플·구글 이름으로 결제가 찍혀 있다면 통신사 제휴 결제나 앱스토어 결제일 수 있어 해당 업체 고객센터에서 따로 내역을 확인해야 하고, 본인이 다시 시작한 적 없는 결제라면 비밀번호를 바꾸고 모든 기기에서 로그아웃까지 함께 해 둡니다.",
      ],
      keywords: [
        "넷플릭스 해지했는데 결제",
        "넷플릭스 취소했는데 돈나감",
        "넷플릭스 해지 후 결제",
        "넷플릭스 모르는 결제",
        "넷플릭스 환불",
      ],
      quickSummary: [
        "먼저 넷플릭스 [계정]에서 해지 완료 여부와 결제 날짜를 확인하세요.",
        "해지가 끝나지 않았거나 누군가 계정을 다시 시작했다면 결제가 이어질 수 있어요.",
      ],
      steps: [
        "넷플릭스 [계정]에서 멤버십 상태와 다음 결제 날짜를 봅니다.",
        "[결제 상세 정보]에서 실제로 빠져나간 날짜와 금액을 확인합니다.",
        "멤버십이 아직 이용 중이면 먼저 [멤버십 해지]를 끝냅니다.",
        "내가 다시 시작하지 않았다면 비밀번호를 바꾸고 모든 기기에서 로그아웃합니다.",
        "결제 상세와 해지 기록을 준비해 넷플릭스 고객센터의 채팅으로 환불 가능 여부를 문의합니다.",
      ],
      tips: [
        "해지했다고 생각한 날짜와 실제 해지 완료 날짜가 다를 수 있어요. 해지 확인 메일도 함께 찾아보세요.",
        "카드 명세에 넷플릭스가 아닌 통신사·애플·구글로 찍혔다면 그 결제 업체에서 내역을 확인해야 해요.",
      ],
      officialUrl: "https://help.netflix.com/ko/node/41049",
      officialActionLabel: "넷플릭스 결제내역 확인하기",
      faq: [
        {
          question: "해지했는데도 또 결제된 이유는?",
          answer:
            "해지 신청과 실제 해지 완료 시점이 다르거나, 다른 사람이 계정을 재구독했을 가능성이 있어요.",
        },
        {
          question: "환불이 잘 안 되는 편인가요?",
          answer:
            "넷플릭스는 이용 기간에 따라 환불 여부가 갈리는 경우가 많아요. 채팅 상담으로 개별 확인이 필요해요.",
        },
        {
          question: "결제처가 넷플릭스가 아니라 통신사로 찍혀요",
          answer:
            "통신사 제휴 결제(이동통신 요금 합산 등)일 수 있어요. 그 통신사 고객센터에서도 확인해야 해요.",
        },
      ],
      lastChecked: "2026-08-20",
    },
    {
      slug: "payment-method",
      title: "결제수단 확인·변경",
      intro: [
        "결제수단을 확인하려는 경우든 다른 카드로 바꾸려는 경우든 경로는 같습니다. 계정 화면에서 결제 수단 관리를 열고, 결제 수단 등록에서 새 카드나 결제수단을 입력한 뒤 그 결제수단이 기본으로 표시되는지까지 확인하면 처리 순서가 끝납니다.",
        "이때 결제수단을 지우는 것과 멤버십을 해지하는 것은 서로 다른 절차라는 점에 주의해야 합니다. 결제수단만 삭제해서는 구독이 끝나지 않으며, 넷플릭스 이용 자체를 그만두려면 계정 화면에서 멤버십 해지를 별도로 눌러야 합니다.",
      ],
      keywords: [
        "넷플릭스 결제수단 변경",
        "넷플릭스 카드 변경",
        "넷플릭스 결제 카드 확인",
        "넷플릭스 결제 오류",
      ],
      quickSummary: [
        "넷플릭스 [계정]의 [결제 수단 관리]에서 현재 카드와 새 결제수단을 확인하세요.",
        "통신사나 앱스토어로 결제 중이면 넷플릭스가 아니라 그 결제 업체에서 바꿔야 합니다.",
      ],
      steps: [
        "넷플릭스에 로그인하고 [계정]을 엽니다.",
        "[결제 수단 관리]를 누릅니다.",
        "[결제 수단 등록]에서 새 카드나 결제수단을 넣습니다.",
        "새 결제수단이 기본으로 표시되는지 확인합니다.",
      ],
      tips: [
        "결제 업체가 통신사·애플·구글로 표시되면 해당 업체의 결제수단을 바꿔야 해요.",
        "결제수단만 지우는 것은 멤버십 해지가 아니에요. 구독을 끝내려면 [멤버십 해지]도 따로 눌러야 합니다.",
      ],
      officialUrl: "https://www.netflix.com/account",
      officialActionLabel: "넷플릭스 계정 열기",
      officialLinkType: "login",
      faq: [
        {
          question: "결제수단을 삭제하면 멤버십도 자동으로 해지되나요?",
          answer:
            "아니요. 결제수단 삭제와 멤버십 해지는 별개의 절차입니다. 구독 자체를 끝내려면 결제수단만 지우지 말고 계정 화면에서 멤버십 해지를 따로 눌러야 합니다.",
        },
        {
          question: "결제 업체가 통신사나 애플·구글로 표시되는데 넷플릭스에서 바로 바꾸면 되나요?",
          answer:
            "아니요. 결제 업체가 통신사·애플·구글로 표시된다면 넷플릭스 화면이 아니라 해당 결제 업체 쪽에서 결제수단을 변경해야 합니다.",
        },
      ],
      lastChecked: "2026-08-19",
    },
  ],
};
